...
self-hosted **Gemma** endpoint is the default, and a **FreeSolo Flash** adapter
SFT-trained specifically on this task (``server/finetune/``) is also wired.
Gemini / Backboard (SAV-51/52) drop in later without touching any call site —
every backend implements the same :class:`LLMClient` protocol.
...
it. The Flash-served adapter needs no such flag.
"""
...
class FreesoloClient:
    """:class:`LLMClient` for a FreeSolo Flash fine-tuned adapter's OpenAI-compatible
    serving endpoint (``server/finetune/``: SFT-trained on this exact recap task).

    POSTs to ``{base_url}/chat/completions`` with ``Bearer`` auth. No special
    request flags are needed — unlike Gemma, the deployed adapter needs no
    ``chat_template_kwargs`` to return content.
    """
...
def get_llm_client(settings: Settings | None = None) -> LLMClient:
    ...
    ``gemma`` and ``freesolo`` are implemented today; ``gemini``/``backboard`` are
    ...
    if backend == "freesolo":
        return FreesoloClient(
            base_url=settings.freesolo_base_url,
            api_key=settings.freesolo_api_key,
            model=settings.freesolo_model,
        )
    raise NotImplementedError(
        f"recap_backend={backend!r} is not wired yet — only 'gemma'/'freesolo' are "
        "implemented. Gemini / Backboard land in SAV-51/52; they are OpenAI-compatible, "
